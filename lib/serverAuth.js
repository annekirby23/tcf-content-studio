...
export async function getSession(req) {
  ...
  // Track last seen — write at most once per hour per user to keep Redis writes low
  try {
    const rateLimitKey = `tcf:lastseen:rl:${userId}`;
    const already = await redis.get(rateLimitKey);
    if (!already) {
      await redis.set(`tcf:lastlogin:${userId}`, new Date().toISOString());
